...
def get_dls(params):
    ...
    elif params.dset == 'test_norm':
        root_path = 'data/test_norm/'
        size = [params.context_points, 0, params.target_points]
        data_id = getattr(params, 'data_id', 'FD001')
        dls = DataLoaders(
                datasetCls=Dataset_TrainTestCSV,
                dataset_kwargs={
                'root_path': root_path,
                'train_path': f'train_norm_{data_id}.csv',
                'test_path': f'test_norm_{data_id}.csv',
                'features': params.features,
                'scale': True,
                'size': size,
                'time_col_name': getattr(params, 'time_col_name', 'Cycle'),
                'unit_col_name': getattr(params, 'unit_col_name', 'UnitNumber'),
                'use_time_features': params.use_time_features,
                "half": params.is_half,
                'all': getattr(params, 'is_all', 0),
                },
                batch_size=params.batch_size,
                workers=params.num_workers,
                )
